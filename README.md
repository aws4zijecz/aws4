...
```
#### Options:
- `-v`: (optional) turns on a verbose output
- `-f`: (optional) forces upload disregarding timestamps
- `-h`: (optional) print script usage and exit
...
This also creates two outputs `aws5_key_id` and `aws5_key_secret`. These are sensitive outputs so to access them run following commands.
```BASH
...
Superset is deployed on a VPC created using a public Terraform module _(file: step_5.vpc.tf)_ with AWS CloudFormation _(file: step_5.formation.tf)_. As the template for this is bigger than Terraform can support directly, it's stored in an S3 bucket. (See: _Deliverables: Design Decisions_ for details.)
...
```
Use commands above for certificate creation and upload or use a `cert.sh` script.

After the deployment is done, URL of the Superset is in Terraform output `SupersetConsole`.

## 6.
>**TASK:** _Configure Superset to be able to query the aforementioned S3 bucket._

After logging into Superset another sensitive Terraform output is used for DB connection:
...
With datasets loaded, it's up to user to define his dashboards and charts. For this example two charts are created:
A pie chart of **Absolute Multiplier Distribution** and scatter chart with **Time Development of Average Multiplier by Month**.
...
- As we can see from the scatter chart Multiplier mechanics wasn't introduced to Mega Lottery until 2011 as the chart shows values of one for any prior dates.
- After the Multiplier was introduced the average Multiplier agregated over a month was rising until end of 2017, but starting 2018 there was a steep decline.
...
For deliverables 1, 2 and 4 see attachements in the email.
...
- use `policy1.json` instead of blank `"*"` admin policy to show, that even admin account can be limited for purposes of its tasks

### Task overview:
1. Create a KMS encrypted S3 bucket - straightforward creation in Terraform
2. Load CSV to S3 - I decided to write a BASH script for this mainly to present my scripting
...
- Public Terraform modules - used for VPC creation as a PoC, otherwise discarded for unsupported `entryPoint` and poor volume mangement
...
Below there are two approaches to this task. Join all numbers and count frequency on all of them together, or count frequency on each column separatedly, join them afterwards and sum the frequencies.
In this case the first approach is faster (due to the number distribution), but for other or larger sets the second approch might become more resource and time effective.

> **WARN:** These are not _"true"_ top 5 numbers as if any number shares the fifth place it will be omitted. To include all numbers that share fifth place see a _deliverable 5.b.iii - Top 5 Mega Ball numbers_, where this is shown, how to solve.
#### Approach 1
```SQL
...
```
#### Approach 2
We can prove this by actually mapping `draw_date` to days of week.
